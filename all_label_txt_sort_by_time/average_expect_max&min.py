...
for plant_type in plant_types:
    ...
    if combined_dates:
        ...
        for date in unique_dates:
            ...
            # 计算去掉最大值和最小值后的平均点云数
            if len(points_on_date) > 2:
                points_on_date.remove(max(points_on_date))
                points_on_date.remove(min(points_on_date))
...
if plant_dates and plant_averages:
    ...
    plt.xlabel('日期')
    plt.ylabel('去极值后的平均点云数')
    plt.xticks(rotation=45)
    plt.grid(True)
    plt.legend(loc='upper right')

    # 保存图像
    plt.savefig('D:\\pcl\\PCL1\\all_single_ply_sort_by_time\\all_plants_average_points_over_time.png')
    plt.show()
else:
    print("没有足够的数据进行绘图。")
