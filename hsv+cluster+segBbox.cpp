...
void euclideanClustering(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud, std::vector<pcl::PointIndices>& cluster_indices) {
    ...
    ec.setClusterTolerance(0.03); // 聚类时两点之间的最大距离//0.08before,0.05,0.025
...
int main(int argc, char** argv) {
    ...
    // 原始文件和新文件的前缀
    std::string prefix = "./0713/0713_";
    std::string prefix_after = "./0713_single/0713_";
    ...
    for (const auto& pattern : patterns) {
        ...
        for (int i = 0; i < cluster_indices.size(); i++) {
            ...
            // 定义边界框的最小和最大坐标
            float min_x = minPt.x - 0.03;
            float min_y = minPt.y - 0.03;
            float min_z = minPt.z - 0.03;
            float max_x = maxPt.x + 0.03;
            float max_y = maxPt.y + 0.03;
            float max_z = maxPt.z + 0.03;
