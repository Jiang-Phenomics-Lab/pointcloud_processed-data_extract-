...
void euclideanClustering(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud, std::vector<pcl::PointIndices>& cluster_indices) {
    ...
    ec.setClusterTolerance(0.05); // 聚类时两点之间的最大距离//0.08before,0.05,0.025
    ec.setMinClusterSize(100); // 聚类的最小尺寸//500before
...
int main(int argc, char** argv) {
    ...
    std::vector<std::string> patterns = { "heihe43" };
    // 原始文件和新文件的前缀
    std::string prefix = "./0628/0628_";
    std::string prefix_after = "./0628_single/0628_";
    ...
    for (const auto& pattern : patterns) {
        ...
        for (int i = 0; i < cluster_indices.size(); i++) {
            ...
            // 定义边界框的最小和最大坐标
            float min_x = minPt.x - 0.08;
            float min_y = minPt.y - 0.08;
            float min_z = minPt.z - 0.08;
            float max_x = maxPt.x + 0.08;
            float max_y = maxPt.y + 0.08;
            float max_z = maxPt.z + 0.08;
